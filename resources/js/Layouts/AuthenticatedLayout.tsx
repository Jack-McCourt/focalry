...
// Studio Manager sub-sections — surfaced in the sidebar on mobile (the
// horizontal StudioManagerNav is desktop-only). Mirrors StudioManagerNav,
// including the "Documents" group.
type SmItem = { label: string; href: string; pattern: string };
type SmEntry = SmItem | { group: string; items: SmItem[] };

const SM_NAV: SmEntry[] = [
    {
        group: 'Clients',
        items: [
    ...
    },
    {
        group: 'Documents',
        items: [
            { label: 'Proposals', href: '/proposals', pattern: 'proposals.*' },
            { label: 'Contracts', href: '/contracts', pattern: 'contracts.*' },
            { label: 'Questionnaires', href: '/questionnaires', pattern: 'questionnaires.*' },
        ],
    },
    {
        group: 'Payments',
        items: [
            { label: 'Payment links', href: '/packages', pattern: 'packages.*' },
            { label: 'Invoices', href: '/invoices', pattern: 'invoices.*' },
        ],
    },
    {
        group: 'Automation',
...
const SM_PATTERNS = SM_NAV.flatMap((e) => ('items' in e ? e.items.map((i) => i.pattern) : [e.pattern]));
...
// ─── Studio Manager group (sidebar) ───────────────────────────────────────────
// The top link behaves like any nav item; on mobile a chevron expands the
// section list inline so the sub-nav lives in the main navigation.

function SidebarStudioManager({
    href,
    icon: Icon,
    locked,
    onNavigate,
    ...
    const anyActive = SM_PATTERNS.some((p) => route().current(p));
    const [open, setOpen] = useState(anyActive);

    const itemClass = (active: boolean) =>
        `block rounded-md px-3 py-2 text-sm transition ${
            active ? 'bg-sidebar-active font-medium text-sidebar-text-active' : 'text-sidebar-text hover:bg-sidebar-hover hover:text-white'
        }`;

    if (locked) {
        return <SidebarLink href={href} icon={Icon} label="Studio Manager" active={false} locked />;
    }

    return (
        <div>
            <div
                className={`group flex items-center rounded-lg pr-1 text-sm transition-all ${
                    anyActive ? 'bg-sidebar-active text-sidebar-text-active' : 'text-sidebar-text hover:bg-sidebar-hover hover:text-white'
                }`}
            >
                <Link href={href} onClick={onNavigate} className="flex flex-1 items-center gap-3 px-3 py-2.5">
                    <Icon className="h-[18px] w-[18px] shrink-0" />
                    <span className="flex-1 font-medium">Studio Manager</span>
                </Link>
                {/* Expand control — only needed on mobile (desktop uses the tab bar). */}
                <button
                    type="button"
                    onClick={() => setOpen((o) => !o)}
                    className="rounded-md p-1.5 text-zinc-400 hover:text-white lg:hidden"
                    aria-label={open ? 'Collapse Studio Manager' : 'Expand Studio Manager'}
                    aria-expanded={open}
                >
            ...
            {open && (
                <div className="mt-0.5 ml-[1.85rem] space-y-0.5 border-l border-sidebar-border pl-3 lg:hidden">
                    {SM_NAV.map((entry) =>
                        'items' in entry ? (
                            <div key={entry.group} className="pt-1">
                                <p className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">{entry.group}</p>
                                <div className="space-y-0.5 border-l border-sidebar-border pl-2">
                                    {entry.items.map((s) => (
                                        <Link key={s.href} href={s.href} onClick={onNavigate} className={itemClass(route().current(s.pattern))}>
                                            {s.label}
                                        </Link>
                                    ))}
                                </div>
                            ...
                            <Link key={entry.href} href={entry.href} onClick={onNavigate} className={itemClass(route().current(entry.pattern))}>
                                {entry.label}
                            </Link>
                        ),
                    )}
...
// ─── Product switcher + sub-nav (Pixieset-style) ──────────────────────────────
// The top-left dropdown switches between the major product areas; the sidebar
// body then shows that product's own navigation.
...
function ProductSwitcher({ pillars, active, onNavigate }: { pillars: Pillar[]; active: Pillar; onNavigate: () => void }) {
    const [open, setOpen] = useState(false);
    const ref = useRef<HTMLDivElement>(null);
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                type="button"
                onClick={() => setOpen((o) => !o)}
                className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-sm text-white ring-1 ring-white/10 transition hover:bg-sidebar-hover"
                aria-haspopup="menu"
                aria-expanded={open}
            >
                <ActiveIcon className="h-[18px] w-[18px] shrink-0" />
                <span className="flex-1 truncate font-semibold">{active.label}</span>
                <svg className={`h-4 w-4 shrink-0 text-zinc-400 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                </svg>
            </button>

            {open && (
                <div className="absolute left-0 right-0 z-50 mt-1 overflow-hidden rounded-lg border border-sidebar-border bg-[#1f1f1f] p-1 shadow-2xl" role="menu">
                    {pillars.map((p) => {
                        const PIcon = p.icon;
                        ...
                        return (
                            <Link
                                key={p.key}
                                href={p.locked ? route('billing.index') : p.home}
                                onClick={() => {
                                    setOpen(false);
                                    onNavigate();
                                }}
                                ...
                                title={p.locked ? `Upgrade your plan to unlock ${p.label}` : undefined}
                            >
                                <PIcon className="h-[18px] w-[18px] shrink-0" />
                                <span className="flex-1">{p.label}</span>
                                {p.locked && <IconLock className="h-3.5 w-3.5 shrink-0 text-zinc-500" />}
                            </Link>
                        );
                    })}
                </div>
...
function SubNav({ entries, onNavigate }: { entries: SmEntry[]; onNavigate: () => void }) {
    ...
    return (
        <div className="space-y-0.5">
            {entries.map((entry) =>
                'items' in entry ? (
                    <div key={entry.group} className="pt-3 first:pt-0">
                        <p className="px-3 pb-1 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">{entry.group}</p>
                        <div className="space-y-0.5">
                            {entry.items.map((s) => (
                                <Link key={s.href} href={s.href} onClick={onNavigate} className={itemClass(route().current(s.pattern))}>
                                    {s.label}
...
export default function AuthenticatedLayout({
    ...
    // The major product areas, switched from the top-left dropdown. Each carries
    // its own sidebar navigation (`nav`).
    const pillars: Pillar[] = [
        ...
        },
        {
            ...
            label: 'Store',
            icon: IconStore,
            home: route('store.orders.index'),
            patterns: ['store.*'],
            feature: 'store',
            nav: [
                { label: 'Orders', href: route('store.orders.index'), pattern: 'store.orders.*' },
                { label: 'Products', href: route('store.products.index'), pattern: 'store.products.*' },
                { label: 'Coupons', href: route('store.coupons.index'), pattern: 'store.coupons.*' },
                { label: 'Gift cards', href: route('store.gift-cards.index'), pattern: 'store.gift-cards.*' },
                { label: 'Settings', href: route('store.settings.edit'), pattern: 'store.settings.*' },
            ],
        },
        {
            key: 'studio',
            ...
            icon: IconStudio,
            // Home = the overview dashboard, then the CRM sections.
            home: route('dashboard'),
            patterns: ['dashboard', ...SM_PATTERNS],
            feature: 'studio_manager',
            nav: [{ label: 'Dashboard', href: route('dashboard'), pattern: 'dashboard' }, ...SM_NAV],
        },
    ].map((p) => ({ ...p, locked: !has(p.feature) }));

    // Fall back to Studio Manager for account/global pages (billing, settings…)
    // that aren't part of any product area.
    const activePillar =
        pillars.find((p) => p.patterns.some((pat) => route().current(pat))) ??
        pillars.find((p) => p.key === 'studio') ??
        pillars[0];
            ...
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
                ...
                <div className="flex h-14 shrink-0 items-center gap-2 border-b border-neutral-200 bg-white px-4 sm:px-6 lg:px-8">
                    <button
                        ...
                        className="-ml-1 shrink-0 rounded-md p-2 text-neutral-600 hover:bg-neutral-100 lg:hidden"
                    ...
                    {/* Page actions sit inline at the top on desktop. */}
                    {actions && <div className="hidden shrink-0 items-center gap-2 lg:flex">{actions}</div>}
                    {/* Global inbox — reachable from any product area. */}
                    <Link
                        href={route('messages.index')}
                        className={`relative rounded-md p-2 transition ${route().current('messages.*') ? 'bg-neutral-100 text-neutral-900' : 'text-neutral-500 hover:bg-neutral-100 hover:text-neutral-700'}`}
                        aria-label="Messages"
                    >
                ...
                {/* On mobile the actions drop to their own bar below the top bar. */}
                {actions && (
                    <div className="flex shrink-0 flex-wrap items-center justify-end gap-2 border-b border-neutral-200 bg-white px-4 py-2.5 sm:px-6 lg:hidden">
                        {actions}
                    </div>
                )}
