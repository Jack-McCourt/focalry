...
export default function ProjectShareVerify({
    token,
    masked_email,
    sent,
    studio_name,
    ...
    const requestCode = () => sendForm.post(route('projects.public.code', token), { preserveScroll: true });
    const submitCode = (e: React.FormEvent) => {
        e.preventDefault();
        codeForm.post(route('projects.public.verify', token), { preserveScroll: true });
    };

    return (
        <PublicShell brand={{ name: studio_name, logo: studio_logo, size: studio_logo_size }} maxWidth="sm">
            <Head title="Verify to view" />
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-8 text-center shadow-sm">
                ...
                <h1 className="mt-4 text-xl font-semibold text-neutral-900">These details are private</h1>
                <p className="mt-2 text-sm text-neutral-500">
                    Enter the code sent to <span className="font-medium text-neutral-700">{masked_email}</span> to view it.
                </p>
                ...
                <form onSubmit={submitCode} className="mt-6 space-y-3 text-left">
                    ...
                    <button type="submit" disabled={codeForm.processing} className="btn-primary w-full">
                        {codeForm.processing ? 'Verifying…' : 'View details'}
                    </button>
                </form>
